...
export function StudentTaskPage() {
	...
	const canAdd = task
		? now >= task.start_at &&
			now <= task.end_at &&
			records.length < task.max_records_per_student
		: false;
	...
	return (
		<StudentPageFrame
			...
			description={
				task
					? `开始：${formatDateTime(task.start_at, "-", clientOffsetMs)}，截止：${formatDateTime(task.end_at, "-", clientOffsetMs)}`
					: ""
			}
			action={
				task && canAdd ? (
		...
		>
						...
						<CardContent className="space-y-3">
							...
							<div className="flex flex-wrap gap-2 text-sm">
								...
								<Badge variant="outline">
									最多 {task.max_records_per_student} 条记录
								</Badge>
							</div>
						</CardContent>
					</Card>
												...
												<PhotoSwipeImageGallery
													...
													images={[
														...
														...record.image_paths
															.filter(
																(imagePath) =>
																	imagePath !== record.cover_image_path,
															)
															.map((imagePath) => ({
																src: imagePath,
																alt: record.title,
											...
											<div className="flex flex-wrap gap-2">
												...
												record.status === "rejected" ? (
													<Button size="sm" variant="outline" asChild>
														<Link
															to={`/student/tasks/${task.id}/upload?id=${record.id}`}
														>
			...
			<ConfirmActionDialog
				...
				title="确认删除记录"
				description={
					deleteTarget ? `《${deleteTarget.title}》将被永久删除。` : ""
				}
				confirmLabel="删除"
				...
				onConfirm={async () => {
					...
					try {
						...
						await unwrapResponse(
							createApiClient()
								.student.records({ id: deleteTarget.id })
								.delete(),
						);
