...
function getStudentSearchPlaceholder(field: StudentSearchField) {
	if (field === "uid") return "搜索 UID 完全匹配的学生";
	if (field === "english_name") return "搜索英文名";
	if (field === "class") return "搜索该班级的学生";
	return "搜索中文名";
}
...
function AdminStudentListPage() {
	...
	const [deleteTarget, setDeleteTarget] =
		useState<StudentWithClassSummary | null>(null);
	...
	const allSelected =
		sortedStudents.length > 0 && selectedIds.length === sortedStudents.length;
	...
	async function updateSelectedClass() {
		...
		try {
			await unwrapResponse(
				createApiClient().admin.students.class.patch({
					ids: selectedIds,
