				...
				>
					...
					<div className="grid gap-4 md:grid-cols-2">
						<Field label="开始时间">
							<DateTimePickerField
								value={form.start_at}
								defaultDate={getServerUtcDateInputValue(clientOffsetMs)}
								onChange={(value) => onFormChange({ ...form, start_at: value })}
								required
							/>
						</Field>
						<Field label="截止时间">
							<DateTimePickerField
								value={form.end_at}
								defaultDate={getServerUtcDateInputValue(clientOffsetMs)}
								onChange={(value) => onFormChange({ ...form, end_at: value })}
								required
							/>
						</Field>
					</div>
					<div className="grid gap-4 md:grid-cols-3">
						<Field label="最少字数">
							<Input
								...
								onChange={(event) =>
									onFormChange({ ...form, min_words: event.target.value })
								}
							/>
						</Field>
						<Field label="最少图片数量">
							<Input
								...
								onChange={(event) =>
									onFormChange({ ...form, min_images: event.target.value })
								}
							/>
						</Field>
						<Field label="每人最多记录数">
							<Input
								type="number"
								min="1"
								value={form.max_records_per_student}
								onChange={(event) =>
									onFormChange({
										...form,
										max_records_per_student: event.target.value,
									})
								}
							/>
						</Field>
					...
					{showScoreEnabled ? (
						<Field label="启用打分">
							<Switch
								checked={form.score_enabled}
								onCheckedChange={(checked) =>
									onFormChange({ ...form, score_enabled: checked })
								}
							/>
						</Field>
					) : null}
					<UserMultiCombobox
						...
						onChange={(value) => {
							const removedClassIds = lockedClassIds.filter(
								(classId) => !value.includes(classId),
							);
