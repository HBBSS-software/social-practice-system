...
export function toUserOption(
	user: Pick<UserSummary, "id" | "name" | "uid">,
): UserOption {
	return {
	...
	return (
		<Field label={label}>
			<Select
				...
				value={resolvedValue}
				onValueChange={(nextValue) =>
					onChange(nextValue === "__all__" ? "" : nextValue)
				}
			>
				<SelectTrigger className="w-full">
...
export const UserMultiCombobox = memo(function UserMultiCombobox({
	...
	const selectedOptions = useMemo(
		() =>
			value
				.map((id) => selectedOptionMap.get(String(id)))
				.filter((option): option is UserOption => Boolean(option)),
		[selectedOptionMap, value],
	);
		...
		<Field label={label}>
			...
			>
				<ComboboxChips ref={anchorRef} className="min-h-9 w-full">
					...
					<ComboboxChipsInput
						placeholder={selectedOptions.length > 0 ? "" : `筛选${label}`}
					/>
...
export const StudentMultiCombobox = memo(function StudentMultiCombobox({
	...
	const visibleGroups = useMemo(() => {
		const groupMap = new Map<
			string,
			{ value: string; items: StudentOption[] }
		>();

		for (const option of visibleOptions) {
			const groupKey = option.class_id
				? String(option.class_id)
				: "__unassigned__";
			const groupLabel =
				option.class_id && option.class_name ? option.class_name : "未分配";
	...
	return (
		<Field label={label}>
			<Combobox
				...
				isItemEqualToValue={(
					item: { value: string; items?: StudentOption[] },
					selected: StudentOption,
				) => !item.items && item.value === selected.value}
			>
				<ComboboxChips ref={anchorRef} className="min-h-9 w-full">
					{selectedOptions.map((option) => (
						<ComboboxChip key={option.value}>{option.label}</ComboboxChip>
		...
		<div className="space-y-4">
			...
			{record.image_paths.length > 0 ? (
				<PhotoSwipeImageGallery
					...
					images={record.image_paths.map((imagePath) => ({
						src: imagePath,
						alt: record.title,
					}))}
				>
