...
export function TeacherTasksPage() {
	...
	const [editingTask, setEditingTask] = useState<PracticeTaskDetail | null>(
		null,
	);
	const [deleteTarget, setDeleteTarget] = useState<PracticeTaskSummary | null>(
		null,
	);
	...
	async function load() {
		...
		try {
			...
			const [taskData, classData] = await Promise.all([
				unwrapResponse<{ tasks: PracticeTaskSummary[] }>(
	...
	const grouped = useMemo(
		() => ({
			active: sortTasks(
				"active",
				tasks.filter((task) => getTaskTab(task, clientOffsetMs) === "active"),
			),
			upcoming: sortTasks(
				"upcoming",
				tasks.filter((task) => getTaskTab(task, clientOffsetMs) === "upcoming"),
			),
			ended: sortTasks(
				"ended",
				tasks.filter((task) => getTaskTab(task, clientOffsetMs) === "ended"),
			),
		}),
	...
	async function openEdit(taskId: number) {
		try {
			const data = await unwrapResponse<{ task: PracticeTaskDetail }>(
				createApiClient().teacher.tasks({ id: taskId }).get(),
			);
		...
		>
			{loading ? (
				...
				<Tabs defaultValue="active">
					...
					<TaskList
						...
						onOpen={(task) => navigate(`${basePath}/${task.id}`)}
						onEdit={openEdit}
					...
					const counts = await Promise.all(
						targetClasses.map(async (targetClass) => {
							const data = await unwrapResponse<{ count: number }>(
								createApiClient()
									.teacher.tasks({ id: editingTask.id })
				...
				onSubmit={async () => {
					try {
						if (editingTask) {
							...
							await unwrapResponse(
								createApiClient().teacher.tasks.post(
									formToPayload(form, clientOffsetMs),
								),
							);
						}
			...
			<ConfirmActionDialog
				...
				onConfirm={async () => {
					...
					await unwrapResponse(
						createApiClient().teacher.tasks({ id: deleteTarget.id }).delete(),
					);
						...
						current
							? {
									...
									classes: current.classes.filter(
										(item) => !removedClassIds.has(item.id),
									),
								}
							: current,
					);
							...
							<CardContent className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
								<div className="min-w-0 space-y-2">
									<div className="flex flex-wrap items-center gap-2">
										...
										<Badge
											variant={
												task.pending_count > 0 ? "destructive" : "secondary"
											}
										>
											{task.pending_count} 待审核
										</Badge>
									</div>
									<div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground">
										<span>
											开始：{formatDateTime(task.start_at, "-", clientOffsetMs)}
										</span>
										<span>
											截止：{formatDateTime(task.end_at, "-", clientOffsetMs)}
										</span>
