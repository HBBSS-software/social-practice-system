...
export function localDateBoundaryIso(
	value: string,
	boundary: "start" | "end",
	clientOffsetMs = 0,
) {
...
export function formatDateTime(
	...
	return isValidDate(parsed)
		? formatUtcDateTimeMinute(applyClientOffset(parsed, clientOffsetMs))
		: fallback;
}
...
export function formatDateTimeInputValue(value: string, clientOffsetMs = 0) {
	...
	return `${[
		...
		"-",
	)}T${padDatePart(shifted.getUTCHours())}:${padDatePart(shifted.getUTCMinutes())}`;
}
