...
test("normalizeDateInputValue extracts UTC date from ISO datetime strings", () => {
	expect(normalizeDateInputValue("2026-03-28T08:30:00.000Z")).toBe(
		"2026-03-28",
	);
});

test("normalizeDateInputValue formats Date instances safely", () => {
	expect(normalizeDateInputValue(new Date(Date.UTC(2026, 2, 28)))).toBe(
		"2026-03-28",
	);
});
...
test("localDateBoundaryIso subtracts client offset from selected date boundaries", () => {
	expect(localDateBoundaryIso("2026-03-28", "start", 8 * 60 * 60 * 1000)).toBe(
		"2026-03-27T16:00:00.000Z",
	);
	expect(localDateBoundaryIso("2026-03-28", "end", 8 * 60 * 60 * 1000)).toBe(
		"2026-03-28T15:59:59.999Z",
	);
	expect(localDateBoundaryIso("2026-03-28", "start", -5 * 60 * 60 * 1000)).toBe(
...
test("formatDateTime formats ISO string with client offset to minutes", () => {
	expect(
		formatDateTime("2026-03-28T08:30:00.000Z", "-", 8 * 60 * 60 * 1000),
	).toBe("2026-03-28 16:30");
	expect(
		formatDateTime("2026-03-28T08:30:00.000Z", "-", -5 * 60 * 60 * 1000),
	).toBe("2026-03-28 03:30");
});

test("datetime input helpers use client offset", () => {
	expect(
		formatDateTimeInputValue("2026-03-28T08:30:00.000Z", 8 * 60 * 60 * 1000),
	).toBe("2026-03-28T16:30");
	expect(
		formatDateTimeInputValue("2026-03-28T08:30:00.000Z", -5 * 60 * 60 * 1000),
	).toBe("2026-03-28T03:30");
	expect(dateTimeInputValueToIso("2026-03-28T16:30", 8 * 60 * 60 * 1000)).toBe(
		"2026-03-28T08:30:00.000Z",
	);
	expect(dateTimeInputValueToIso("2026-03-28T03:30", -5 * 60 * 60 * 1000)).toBe(
		"2026-03-28T08:30:00.000Z",
	);
...
test("server now helpers subtract client offset", () => {
	...
	try {
		expect(getServerNowIso(8 * 60 * 60 * 1000)).toBe(
