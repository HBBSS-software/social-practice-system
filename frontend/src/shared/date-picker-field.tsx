			...
			<Popover open={open} onOpenChange={setOpen}>
				<PopoverTrigger asChild>
					<Button
						...
						variant="outline"
						className={cn(
							"relative w-full justify-start px-3 text-left font-normal",
		...
		<div className={cn("flex w-full flex-col gap-2 sm:flex-row", className)}>
			<Popover open={open} onOpenChange={setOpen}>
				<PopoverTrigger asChild>
					<Button
						type="button"
						variant="outline"
						className="min-w-0 flex-1 justify-start gap-2 px-3 text-left font-normal"
					>
						<span
							className={cn(
								"min-w-0 flex-1 truncate",
				...
				{open ? (
					<PopoverContent align="start" className="w-auto overflow-hidden p-0">
						<Calendar
							...
							onSelect={(nextDate) => {
								onChange(
									toDateTimeValue(toDateText(nextDate), time || "00:00"),
								);
								setOpen(false);
							}}
						/>
			...
			<Input
				...
				onChange={(event) => {
					...
					onChange(
						toDateTimeValue(
							date || defaultDate || normalizeDateInputValue(new Date()),
							nextTime,
						),
					);
				}}
