...
export const tmpUploadPathPattern =
	/^\/tmp-uploads\/[A-Za-z0-9][A-Za-z0-9._-]*$/;
...
export function toPracticeRecord(
	row: typeof practiceRecords.$inferSelect,
): PracticeRecord {
	...
	return {
		...
		cover_image_path:
			row.coverImagePath && imagePaths.includes(row.coverImagePath)
				? row.coverImagePath
				: (imagePaths[0] ?? null),
...
export function toStudentSummary(
	row: Pick<UserRow, "id" | "name" | "englishName" | "createdAt">,
) {
	return {
		id: row.id,
		uid: row.id,
...
export function toUserSummary(
	row: Pick<UserRow, "id" | "role" | "name" | "englishName" | "createdAt">,
): UserSummary {
	return {
		id: row.id,
		uid: row.id,
...
export function toClassSummary(
	row: Pick<ClassRow, "id" | "name" | "createdAt">,
) {
	return {
		id: row.id,
		name: row.name,
...
export function parseImagePaths(value: string | null) {
	...
	try {
		...
		return Array.isArray(parsed)
			? parsed.filter(
					(item): item is string =>
						typeof item === "string" && uploadPathPattern.test(item),
				)
			: [];
	} catch {
		return [];
...
export function parseRecordImagePaths(value: string | null) {
	...
	try {
		...
		return Array.isArray(parsed)
			? parsed.filter(
					(item): item is string =>
						typeof item === "string" && tmpUploadPathPattern.test(item),
				)
			: [];
	} catch {
		return [];
	...
	if (filters.student_ids) {
		conditions.push(
			filters.student_ids.length > 0
				? inArray(practiceRecords.studentId, filters.student_ids)
				: sql`1 = 0`,
		);
	}
	if (filters.class_id) {
		conditions.push(
			sql`${practiceRecords.studentId} in (select ${classStudents.studentId} from ${classStudents} where ${classStudents.classId} = ${filters.class_id})`,
		);
	}
...
export function recordIdentitySelect() {
	return {
		...
		student_english_name: sql<
			string | null
		>`case when ${users.id} is null or ${users.deletedAt} is not null then null else ${users.englishName} end`,
	};
}
