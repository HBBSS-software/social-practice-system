...
export function deleteUser(id: number) {
	...
	db.transaction((tx) => {
		tx.update(practiceRecords)
			...
			.where(
				and(
					eq(practiceRecords.studentId, user.id),
					sql`${practiceRecords.studentUidSnapshot} is null`,
				),
			)
			.run();
		if (user.role === "teacher")
			tx.delete(classTeachers)
