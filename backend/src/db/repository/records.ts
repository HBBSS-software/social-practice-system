...
import {
	buildRecordWhere,
	deletedUserName,
	normalizeRecordImagePaths,
	nowIso,
	practiceRecordColumns,
	recordHasImagePathCondition,
	recordIdentitySelect,
	serializeImagePaths,
	tmpUploadPathPattern,
	toFiniteNumber,
	toPracticeRecord,
	uploadPathPattern,
} from "../helpers.js";
...
import {
	createUploadPathFromSource,
	removeUploadFile,
	resolveTmpUploadFilePath,
...
export function createRecord(input: CreateRecordInput) {
	const student = db
		.select({ id: users.id })
		.from(users)
		.where(eq(users.id, input.student_id))
		.get();
...
export function canAccessUpload(
	...
	return Boolean(
		db
			...
			.innerJoin(
				sql`class_students`,
				eq(practiceRecords.studentId, sql`class_students.student_id`),
			)
			.innerJoin(
				sql`class_teachers`,
				eq(sql`class_students.class_id`, sql`class_teachers.class_id`),
			)
...
export function getRecordsByStudentTask(
	studentId: number,
	taskId: number,
): StudentRecord[] {
	return db
		...
		.where(
			and(
				...
				eq(practiceRecords.taskId, taskId),
			),
		)
		.orderBy(desc(practiceRecords.createdAt))
		.all()
...
export function getTeacherRecordById(
	id: number,
...
export function getRecordsForExport(
	...
	return db
		.select({
			...
			content: practiceRecords.content,
			practice_date: practiceRecords.practiceDate,
			location: practiceRecords.location,
			duration: practiceRecords.duration,
			status: practiceRecords.status,
			score: practiceRecords.score,
			teacher_comment: practiceRecords.teacherComment,
		...
		.leftJoin(
			classStudents,
			eq(practiceRecords.studentId, classStudents.studentId),
		)
		...
		.map((row) => ({
			...
			content: row.content,
			image_count: normalizeRecordImagePaths(row.image_paths).length,
		}));
}
...
export function updateRecord(id: number, updates: UpdateRecordInput) {
	...
	if (updates.practice_date !== undefined)
		nextValues.practiceDate = updates.practice_date;
	...
	if (updates.teacher_comment !== undefined)
		nextValues.teacherComment = updates.teacher_comment;
...
export function countStudentRecordsToday(studentId: number) {
	const row = db
		...
		.where(
			and(
				eq(practiceRecords.studentId, studentId),
				gte(practiceRecords.createdAt, startOfUtcTodayIso()),
			),
		)
		.get();
	return toFiniteNumber(row?.count);
...
function prepareRecordImages(
	imagePathsInput: string[],
	coverImagePathInput: string | null | undefined,
	reusableImagePaths: string[] = [],
) {
	const sourceImagePaths = [...new Set(imagePathsInput)].slice(
		0,
		MAX_RECORD_IMAGES,
	);
	...
	for (const sourceImagePath of sourceImagePaths) {
		...
		db.delete(tempUploadDeletions)
			.where(eq(tempUploadDeletions.filePath, sourceImagePath))
			.run();
		movedImagePaths.set(sourceImagePath, target.imagePath);
		imagePaths.push(target.imagePath);
	}
	const coverImagePath =
		coverImagePathInput && movedImagePaths.has(coverImagePathInput)
...
function buildRecordOrderBy(sort: RecordSort) {
	...
	if (sort === "score_desc")
		return [
			sql`case when ${practiceRecords.score} is null then 1 else 0 end`,
			desc(practiceRecords.score),
			desc(practiceRecords.createdAt),
		];
	if (sort === "score_asc")
		return [
			sql`case when ${practiceRecords.score} is null then 1 else 0 end`,
			asc(practiceRecords.score),
			desc(practiceRecords.createdAt),
		];
