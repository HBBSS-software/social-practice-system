...
import { buildRecordWhere, nowIso, toFiniteNumber } from "../helpers.js";
import {
	classes,
	classStudents,
	practiceRecords,
...
import { getClasses, getStudentClassId } from "./classes.js";
...
export function calculateRecordStatistics(
	where?: ReturnType<typeof buildRecordWhere> | ReturnType<typeof eq>,
) {
	const row = db
		.select({
...
export function getStudentDashboardOverview(
	...
	const activeTaskRow = classId
		? db
				...
				.innerJoin(
					practiceTaskClasses,
					eq(practiceTaskClasses.taskId, practiceTasks.id),
				)
				.where(
					and(
						eq(practiceTaskClasses.classId, classId),
						lte(practiceTasks.startAt, now),
						gte(practiceTasks.endAt, now),
					),
				)
				.get()
		: null;
...
export function getStatistics(
	...
	if (visibleStudentIds) {
		...
		studentConditions.push(
			ids.length > 0 ? inArray(users.id, ids) : sql`1 = 0`,
		);
	}
	...
	const recordStats = calculateRecordStatistics(
		buildRecordWhere({}, visibleStudentIds),
	);
...
export function getOverview(
	visibleClassIds?: Set<number>,
	selectedClassId: number | null = null,
): OverviewData {
	const classIds = visibleClassIds
		? [...visibleClassIds]
		: getClasses().map((item) => item.id);
	const scopedClassIds = selectedClassId
		? classIds.filter((classId) => classId === selectedClassId)
		: classIds;
	const visibleCondition =
		scopedClassIds.length > 0
			? inArray(classes.id, scopedClassIds)
			: sql`1 = 0`;
	const now = nowIso();
	...
	const classRows = db
		...
		.leftJoin(
			practiceRecords,
			and(
				eq(practiceRecords.taskId, practiceTaskClasses.taskId),
	...
	const classRankings = db
		...
		.groupBy(classes.id)
		.orderBy(
			desc(classOverviewSelection.total_duration),
			desc(classOverviewSelection.total_records),
			asc(classes.name),
		)
	...
	const studentRows = db
		...
		.leftJoin(
			practiceTaskClasses,
			eq(practiceTaskClasses.classId, classStudents.classId),
		)
		.leftJoin(
			practiceRecords,
			and(
				eq(practiceRecords.studentId, users.id),
				eq(practiceRecords.taskId, practiceTaskClasses.taskId),
			),
		)
		.where(
			and(visibleCondition, eq(users.role, "student"), isNull(users.deletedAt)),
		)
		.groupBy(users.id, classes.id)
		.orderBy(
			desc(totalDurationExpression),
			desc(totalRecordsExpression),
			asc(users.name),
			asc(users.id),
		)
		...
		const activeTaskRow = classId
			? db
					...
					.where(
						and(
							eq(practiceTaskClasses.classId, classId),
							lt(practiceTasks.startAt, range.end),
							gte(practiceTasks.endAt, range.start),
						),
					)
					.get()
			: null;
...
function getOverviewTrend(classIds: number[]) {
	...
	return months.map((month) => {
		...
		const classWhere =
			classIds.length > 0
				? inArray(practiceTaskClasses.classId, classIds)
				: sql`1 = 0`;
