...
export function getManageableTaskById(
	taskId: number,
	...
	return task.classes.some((item) => visibleClassIds.has(item.id))
		? task
		: null;
}
...
export function createTask(input: CreatePracticeTaskInput) {
	...
	const result = db.transaction((tx) => {
		...
		tx.insert(practiceTaskClasses)
			.values(classIds.map((classId) => ({ taskId, classId, createdAt })))
			.run();
		return taskId;
	});
...
export function updateTask(taskId: number, input: UpdatePracticeTaskInput) {
	...
	if (input.max_records_per_student !== undefined)
		values.maxRecordsPerStudent = input.max_records_per_student;
	db.transaction((tx) => {
		if (Object.keys(values).length > 0) {
			tx.update(practiceTasks)
				.set(values)
		...
		if (input.class_ids !== undefined) {
			...
			const nextClassIds = uniquePositiveIds(input.class_ids).filter(
			...
			if (nextClassIds.length > 0) {
				...
				tx.insert(practiceTaskClasses)
					.values(
						nextClassIds.map((classId) => ({ taskId, classId, createdAt })),
					)
					.onConflictDoNothing()
					.run();
			}
...
export function removeTaskClass(taskId: number, classId: number) {
	...
	db.transaction((tx) => {
		tx.delete(practiceRecords)
			.where(
				sql`${practiceRecords.id} in (select practice_records.id from practice_records inner join class_students on practice_records.student_id = class_students.student_id where practice_records.task_id = ${taskId} and class_students.class_id = ${classId})`,
			)
			.run();
		tx.delete(practiceTaskClasses)
			.where(
				and(
					eq(practiceTaskClasses.taskId, taskId),
					eq(practiceTaskClasses.classId, classId),
				),
			)
			.run();
...
export function getManageableTasks(
	visibleClassIds?: Set<number>,
): PracticeTaskSummary[] {
	const where = visibleClassIds
		? (() => {
				...
				return ids.length > 0
					? inArray(practiceTaskClasses.classId, ids)
					: sql`1 = 0`;
			})()
		: undefined;
	return db
...
function decorateTask(
	row: typeof practiceTasks.$inferSelect,
	studentId?: number,
): PracticeTaskSummary {
	const classCount = db
		.select({ count: sql<number>`count(*)` })
	...
	const recordStats = calculateRecordStatistics(
		eq(practiceRecords.taskId, row.id),
	);
	const myRecordCount = studentId
		? countStudentTaskRecords(studentId, row.id)
		: undefined;
	return {
		...toPracticeTask(row),
...
export function getTaskDetail(
	...
	if (!row) return null;
	return {
		...decorateTask(row, studentId),
		classes: getClassesForTask(taskId),
	};
}
