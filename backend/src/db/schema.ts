...
export const practiceTasks = sqliteTable(
	"practice_tasks",
	{
		...
		maxRecordsPerStudent: integer("max_records_per_student")
			.notNull()
			.default(1),
		scoreEnabled: integer("score_enabled", { mode: "boolean" })
			.notNull()
			.default(false),
...
export const practiceTaskClasses = sqliteTable(
	"practice_task_classes",
	{
...
export const practiceRecords = sqliteTable(
	...
	{
		id: integer("id").primaryKey({ autoIncrement: true }),
		taskId: integer("task_id").references(() => practiceTasks.id, {
			onDelete: "cascade",
		}),
		studentId: integer("student_id")
			.notNull()
	...
	(table) => [
		...
		index("practice_records_task_student_idx").on(
			table.taskId,
			table.studentId,
		),
...
export const tempUploadDeletions = sqliteTable(
	"temp_upload_deletions",
	{
		id: integer("id").primaryKey({ autoIncrement: true }),
