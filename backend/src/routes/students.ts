...
function buildRecordPayload(body: Record<string, unknown>) {
	...
	const practiceDate =
		typeof body.practice_date === "string" ? body.practice_date.trim() : "";
	const location =
		body.location === undefined
			? undefined
			: normalizeOptionalString(body.location);
	const duration =
		body.duration === undefined ? undefined : parseDuration(body.duration);
	const imagePaths = Array.isArray(body.image_paths)
		? body.image_paths.filter(
...
function validateTaskRecordConstraints(
	task: {
		min_words: number;
		min_images: number;
...
export const studentRoutes = new Hono<AppBindings>()
	...
	.post(
		"/students/me/records",
		zValidator("json", createRecordBodySchema, validationHook),
		(c) => {
			...
			const imageError = validateRecordImages(
				imagePaths,
				payload.coverImagePath ?? null,
			);
			if (imageError) return apiError(c, 400, imageError);

			const taskError = validateTaskRecordConstraints(
				task,
				payload.content,
				imagePaths,
			);
			...
			if (
				database.countStudentTaskRecords(user.id, task.id) >=
				task.max_records_per_student
			) {
				return apiError(
					c,
					400,
					`每个学生最多提交 ${task.max_records_per_student} 条记录。`,
				);
			}

			if (
				database.countStudentRecordsToday(user.id) >= database.MAX_DAILY_RECORDS
			) {
				return apiError(
					c,
					429,
					`每天最多创建 ${database.MAX_DAILY_RECORDS} 条实践记录。`,
				);
			}

			let record: ReturnType<typeof database.createRecord>;
