...
import {
	type CsvUserImportEntry,
	createUserCredentialsCsv,
	parseUserImportCsvBuffer,
} from "../csv/user-import.js";
...
async function readImportFile(file: File) {
	...
	return parseUserImportCsvBuffer(new Uint8Array(await file.arrayBuffer()), {
		columnCount: 4,
	});
}
...
export const adminRoutes = new Hono<AppBindings>()
	...
	.post(
		"/users",
		zValidator("json", createUserBodySchema, validationHook),
		async (c) => {
			...
			try {
				...
				return apiError(
					c,
					400,
					error instanceof Error ? error.message : "班级信息无效。",
				);
			}
		},
	)
	.post(
		"/users/batch",
		zValidator("json", batchCreateUsersBodySchema, validationHook),
		async (c) => {
			const body = c.req.valid("json");
			...
			for (let index = 0; index < body.entries.length; index += 1) {
				...
				if (englishNameError) {
					return apiError(
						c,
						400,
						`第 ${index + 1} 行错误：${englishNameError}`,
					);
				}
				...
				try {
					...
					return apiError(
						c,
						400,
						`第 ${index + 1} 行错误：${error instanceof Error ? error.message : "班级信息无效。"}`,
					);
				}
			}
	...
	.post("/users/import/preview", async (c) => {
		try {
			...
			return c.json({
				...
				entries: entries.map(
					({ classId: _classId, englishName, ...entry }) => ({
						...entry,
			...
			return apiError(
				c,
				400,
				error instanceof Error ? error.message : "CSV 文件无效。",
			);
		}
	})
	.post("/users/import", async (c) => {
		try {
			...
			const parsed = await readImportFile(file);
			const entries = validateImportEntries(parsed.entries).map(
				({ class_name: _className, ...entry }) => entry,
			);
	...
	.get(
		...
		zValidator(
			"query",
			userSearchQuerySchema.extend({ role: userRoleSchema }),
			validationHook,
		),
		(c) => {
	...
	.put(
		...
		zValidator(
			"param",
			z.object({ id: z.string().regex(/^[1-9]\d*$/) }),
			validationHook,
		),
	...
	.delete(
		"/users",
		zValidator("json", batchDeleteUsersBodySchema, validationHook),
		(c) => {
			const body = c.req.valid("json");
	...
	.post(
		"/classes",
		zValidator("json", createClassBodySchema, validationHook),
		(c) => {
			const body = c.req.valid("json");
			const nameError = validateName(body.name);
	...
	.put(
		"/classes/:classId",
		zValidator("param", classIdParamSchema, validationHook),
		zValidator("json", createClassBodySchema, validationHook),
		(c) => {
	...
	.get(
		"/classes/students",
		zValidator("query", classStudentsQuerySchema, validationHook),
		(c) => {
			const query = c.req.valid("query");
			...
			return c.json({
				students: database.searchStudentsForClassAssignment(
					query.q?.trim() ?? "",
					classId,
				),
			});
	...
	.put(
		...
		zValidator("param", classIdParamSchema, validationHook),
		zValidator("json", classTeachersBodySchema, validationHook),
		(c) => {
			const classId = Number(c.req.valid("param").classId);
			const body = c.req.valid("json");
			const targetClass = database.findClassById(classId);
	...
	.put(
		...
		zValidator("param", classIdParamSchema, validationHook),
		zValidator("json", classStudentsBodySchema, validationHook),
		(c) => {
			const classId = Number(c.req.valid("param").classId);
			const body = c.req.valid("json");
			const targetClass = database.findClassById(classId);
