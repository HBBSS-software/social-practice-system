...
function canManageClasses(classIds: number[], userId: number, role: UserRole) {
	...
	if (role === "admin") {
		return classIds.every((classId) =>
			Boolean(database.findClassById(classId)),
		);
	}
...
function parseRecordFilters(query: Record<string, unknown>): RecordFilters {
	return normalizeRecordFilters({
		...
		student_id:
			typeof query.student_id === "string" ? Number(query.student_id) : null,
		student_ids:
			typeof query.student_ids === "string" && query.student_ids
				? query.student_ids.split(",").map(Number)
				: null,
		class_id:
			typeof query.class_id === "string" ? Number(query.class_id) : null,
		class_ids:
			typeof query.class_ids === "string" && query.class_ids
				? query.class_ids.split(",").map(Number)
				: null,
		status:
			typeof query.status === "string"
				? (query.status as RecordFilters["status"])
				: null,
		practice_after:
			typeof query.practice_after === "string" && query.practice_after
				? query.practice_after
				: null,
		practice_before:
			typeof query.practice_before === "string" && query.practice_before
				? query.practice_before
				: null,
		created_after:
			typeof query.created_after === "string" && query.created_after
				? parseDateTimeInput(query.created_after)
				: null,
		created_before:
			typeof query.created_before === "string" && query.created_before
				? parseDateTimeInput(query.created_before)
				: null,
	});
}
...
function buildTaskPayload(body: {
	...
	const startAt =
		body.start_at === undefined ? undefined : parseDateTimeInput(body.start_at);
	const endAt =
		body.end_at === undefined ? undefined : parseDateTimeInput(body.end_at);
...
function validateTaskPayload(
	...
	current?: { start_at: string; end_at: string },
) {
...
function validateRecordImages(
	...
	if (
		imagePaths.some(
			(imagePath) =>
				!isValidRecordImagePath(imagePath) && !isValidUploadPath(imagePath),
		)
	) {
		return "图片路径无效。";
	}
...
export const teacherRoutes = new Hono<AppBindings>()
	...
	.get("/teacher/tasks", (c) => {
		...
		return c.json({
			tasks: database.getManageableTasks(
				getVisibleClassIds(user.id, user.role),
			),
		});
	})
	.post(
	...
	.get(
		"/teacher/tasks/:id",
		zValidator("param", recordIdParamSchema, validationHook),
		(c) => {
			const id = Number(c.req.valid("param").id);
			const user = c.get("user")!;
			const task = database.getManageableTaskById(
				id,
				getVisibleClassIds(user.id, user.role),
			);
	...
	.put(
		"/teacher/tasks/:id",
		zValidator("param", recordIdParamSchema, validationHook),
		zValidator("json", updateTaskBodySchema, validationHook),
		(c) => {
			const id = Number(c.req.valid("param").id);
			const body = c.req.valid("json");
			const user = c.get("user")!;
			...
			if (
				body.class_ids &&
				!canManageClasses(body.class_ids, user.id, user.role)
			) {
				return apiError(c, 403, "无权选择部分班级。");
			}
			...
			const updated = database.updateTask(id, {
				...
				...(payload.start_at === undefined || payload.start_at === null
					? {}
					: { start_at: payload.start_at }),
				...(payload.end_at === undefined || payload.end_at === null
					? {}
					: { end_at: payload.end_at }),
				...(payload.min_words === undefined
					? {}
					: { min_words: payload.min_words }),
				...(payload.min_images === undefined
					? {}
					: { min_images: payload.min_images }),
				...(payload.max_records_per_student === undefined
					? {}
					: { max_records_per_student: payload.max_records_per_student }),
				...(payload.class_ids === undefined
					? {}
	...
	.get(
		"/teacher/tasks/:id/classes/:classId/record-count",
		zValidator(
			"param",
			z.object({
				id: z.string().regex(/^[1-9]\d*$/),
		...
		(c) => {
			...
			if (!task?.classes.some((item) => item.id === classId)) {
	...
	.delete(
		"/teacher/tasks/:id/classes/:classId",
		zValidator(
			"param",
			z.object({
				id: z.string().regex(/^[1-9]\d*$/),
	...
	.post(
		"/teacher/tasks/:id/export",
		zValidator("param", recordIdParamSchema, validationHook),
		zValidator("json", classIdsBodySchema, validationHook),
		async (c) => {
			const id = Number(c.req.valid("param").id);
			const body = c.req.valid("json");
			const user = c.get("user")!;
			...
			const records = database.getRecordsForExport(
				{ task_id: id, class_ids: body.class_ids },
				visibleStudentIds,
			);
			const csv = formatCsv([
				[
	...
	.get(
		...
		(c) => {
			const user = c.get("user")!;
			const classId = c.req.valid("query").class_id
				? Number(c.req.valid("query").class_id)
				: null;
	...
	.get(
		"/teacher/records",
		zValidator("query", recordQuerySchema, validationHook),
		(c) => {
			const query = c.req.valid("query");
			const user = c.get("user")!;
			const filterError = validateRecordFilters(
	...
	.get(
		...
		(c) => {
			const id = Number(c.req.valid("param").id);
			const user = c.get("user")!;
			const record = database.getTeacherRecordById(
				id,
				getVisibleStudentIds(user.id, user.role),
			);
	...
	.get("/teacher/classes", (c) => {
		...
		return c.json({
			classes:
				user.role === "admin"
					? database.getClasses()
					: database.getTeacherClasses(user.id),
		});
	})
	.put(
		"/teacher/records/:id/review",
		zValidator("param", recordIdParamSchema, validationHook),
		zValidator("json", reviewRecordBodySchema, validationHook),
		(c) => {
			const id = Number(c.req.valid("param").id);
			const body = c.req.valid("json");
			const user = c.get("user")!;
			...
			const task = record?.task_id
				? database.getManageableTaskById(
						record.task_id,
						getVisibleClassIds(user.id, user.role),
					)
				: null;
			...
			const scoreError = validateReviewScore({
				status: body.status,
				score: body.score,
				scoreEnabled: Boolean(task?.score_enabled),
			});
	...
	.post(
		"/teacher/record-reviews/batch",
		zValidator("json", batchReviewBodySchema, validationHook),
		(c) => {
			const body = c.req.valid("json");
			const user = c.get("user")!;
			const visibleStudentIds = getVisibleStudentIds(user.id, user.role);
	...
	.put(
		"/teacher/records/:id",
		zValidator("param", recordIdParamSchema, validationHook),
		zValidator("json", updateRecordBodySchema, validationHook),
		(c) => {
			const id = Number(c.req.valid("param").id);
			const body = c.req.valid("json");
			const user = c.get("user")!;
			...
			if (
				...
				const coverImagePath =
					body.cover_image_path !== undefined
						? normalizeOptionalString(body.cover_image_path)
	...
	.get(
		...
		zValidator("query", userSearchQuerySchema, validationHook),
		(c) => {
			const user = c.get("user")!;
			const query = c.req.valid("query");
			...
			return c.json({
				students: database.searchStudents(
					query.q?.trim() ?? "",
					getVisibleStudentIds(user.id, user.role),
					classIds,
					query.field,
	...
	.put(
		"/teacher/students/:id",
		zValidator("param", recordIdParamSchema, validationHook),
		zValidator("json", updateUserBodySchema, validationHook),
		async (c) => {
			const studentId = Number(c.req.valid("param").id);
	...
	.patch(
		...
		zValidator("json", batchUpdateStudentClassBodySchema, validationHook),
		(c) => {
			const body = c.req.valid("json");
			const user = c.get("user")!;
			const ids =
				user.role === "admin"
					? body.ids
					: body.ids.filter((id: number) =>
							canManageStudent(id, user.id, user.role),
						);
	...
	.patch(
		"/teacher/students/password-reset",
		zValidator("json", batchResetPasswordBodySchema, validationHook),
		async (c) => {
			const body = c.req.valid("json");
			const user = c.get("user")!;
			const ids =
