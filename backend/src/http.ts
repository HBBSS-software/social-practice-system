...
export const recordSortSchema = z.enum([
	...
	"score_desc",
	"score_asc",
] satisfies [RecordSort, RecordSort, RecordSort, RecordSort]);
export const notificationTypeSchema = z.enum(notificationTypes);
const recordTitleSchema = z
	.string()
	.min(1, "标题不能为空。")
	.refine((value) => countTextLength(value.trim()) <= RECORD_TITLE_MAX_LENGTH, {
		message: `记录标题不能超过 ${RECORD_TITLE_MAX_LENGTH} 字。`,
	});
const taskTitleSchema = z
	.string()
	.min(1, "任务名称不能为空。")
	.refine((value) => countTextLength(value.trim()) <= TASK_TITLE_MAX_LENGTH, {
		message: `任务名称不能超过 ${TASK_TITLE_MAX_LENGTH} 字。`,
	});
const recordContentSchema = z
	.string()
	.min(1)
	.refine((value) => countTextLength(value.trim()) <= CONTENT_MAX_LENGTH, {
...
const optionalLocationSchema = z
	...
	.refine(
		(value) =>
			value == null || countTextLength(value.trim()) <= LOCATION_MAX_LENGTH,
		{
...
const optionalCommentSchema = z
	...
	.refine(
		(value) =>
			value === undefined ||
			countTextLength(value.trim()) <= COMMENT_MAX_LENGTH,
		{
			message: `评语不能超过 ${COMMENT_MAX_LENGTH} 字。`,
		},
	);
const optionalTaskDescriptionSchema = z
	.string()
	.nullable()
	.optional()
	.refine(
		(value) =>
			value == null || countTextLength(value.trim()) <= CONTENT_MAX_LENGTH,
		{
			message: `任务说明不能超过 ${CONTENT_MAX_LENGTH} 字。`,
		},
	);
const pqSealTextSchema = z
	.string()
	.min(1)
	.max(4096)
	.regex(/^[A-Za-z0-9_-]+$/, "密码格式无效。");
const pqSealEnvelopeSchema = z
	.object(
		{
			v: z.literal(1),
...
export const studentUidLoginBodySchema = z
	.object({
		uid: z.number().int().positive(),
	})
	.merge(pqSealedFieldsSchema);

export const studentNameLoginBodySchema = z
	.object({
...
export const staffLoginBodySchema = z
	.object({
...
export function validationHook(
	...
	return apiError(
		c,
		400,
		result.error?.issues?.[0]?.message ?? "请求参数无效。",
	);
}
