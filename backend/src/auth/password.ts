...
const passwordHashProfiles: Record<
	PasswordHashProfile,
	PasswordHashProfileDefinition
> = {
	standard: {
		id: "standard-v2",
		params: {
...
function formatHash(
	nonce: Buffer,
	tag: Buffer,
	profileId: PasswordHashProfileId,
) {
	return `${hashPrefix}$${profileId}$${nonce.toString("hex")}$${tag.toString("hex")}`;
}
...
function parseHash(value: string) {
	...
	try {
		const profile =
			passwordHashProfilesById[profileId as PasswordHashProfileId];
...
export async function hashPassword(
	password: string,
	profile: PasswordHashProfile = "standard",
) {
...
function hashPasswordsInWorker(
	passwords: string[],
	profileId: PasswordHashProfileId,
	params: Argon2Params,
) {
...
export async function hashPasswords(
	...
	if (workerCount <= 1 || passwords.length < minWorkerBatchSize) {
		return Promise.all(
			passwords.map((password) => hashPassword(password, profile)),
		);
	}
	...
	const hashedChunks = await Promise.all(
		chunks.map((chunk) =>
			hashPasswordsInWorker(chunk, resolvedProfile.id, resolvedProfile.params),
		),
	);
