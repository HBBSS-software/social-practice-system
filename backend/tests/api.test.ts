...
const testUploadDir = fileURLToPath(
	new URL("../data/uploads", import.meta.url),
);
const testTmpUploadDir = fileURLToPath(
	new URL("../data/tmp-uploads", import.meta.url),
);
...
beforeAll(async () => {
	...
	if (!hadFrontendIndex) {
		fs.writeFileSync(
			testFrontendIndex,
			"<!doctype html><html><body>Praxis</body></html>",
		);
	}
...
async function sealPasswordFields<T extends Record<string, unknown>>(
	body: T,
	keys: Array<keyof T>,
) {
...
function createOpenTaskForStudent(studentId: number, scoreEnabled = false) {
	const targetClass = database.createClass(
		`任务测试班级 ${Date.now()} ${Math.random()}`,
	);
...
describe("student search", () => {
	test("searches students by Chinese name, English name, class and UID", async () => {
		...
		const [matchedStudent, otherStudent] = await database.createUsers([
			{
				name: "字段搜索学生甲",
				englishName: "Field Search Alpha",
				role: "student",
				classId: targetClass.id,
			},
			{
				name: "字段搜索学生乙",
				englishName: "Field Search Beta",
				role: "student",
			},
		]);
		...
		async function search(
			query: string,
			field: "name" | "english_name" | "class" | "uid",
		) {
			const response = await apiRequest(
				`/api/teacher/students/search?q=${encodeURIComponent(query)}&field=${field}`,
				{ headers },
			);
			const payload = await readJson(response);
			expect(response.status).toBe(200);
		...
		const partialUidMatches = await search(
			String(matchedStudent!.uid).slice(0, -1) || "0",
			"uid",
		);
		...
		for (const matches of [
			...
			expect(matches.some((student) => student.id === matchedStudent!.id)).toBe(
				true,
			);
		}

		expect(
			englishNameMatches.some((student) => student.id === otherStudent!.id),
		).toBe(false);
		expect(
			partialUidMatches.some((student) => student.id === matchedStudent!.id),
		).toBe(false);
	});
});
...
describe("database bootstrap and users", () => {
	test("creates users and filters by role", async () => {
		...
		expect(database.findUserById(createdStudent.id)?.password).toMatch(
			/^argon2id\$low-v2\$[0-9a-f]+\$[0-9a-f]+$/,
		);
		expect(
			isLowCostPasswordHash(
				database.findUserById(createdStudent.id)?.password ?? "",
			),
		).toBe(true);
		...
		expect(
			database
				...
				.some((user) => user.uid === createdUsers[0]?.uid),
		).toBe(true);
		expect(
			createdUsers[2] ? database.getStudentClassId(createdUsers[2].id) : null,
		).toBe(targetClass.id);
	...
	test("updates, resets and deletes users", async () => {
		...
		expect(database.updateUserPassword(createdUser.id, "hashed-password")).toBe(
			true,
		);
		expect(database.findUserById(createdUser.id)?.password).toBe(
			"hashed-password",
		);
		...
		expect(
			isLowCostPasswordHash(
				database.findUserById(createdUser.id)?.password ?? "",
			),
		).toBe(true);
...
describe("assignments, records and notifications", () => {
	test("assigns teachers and students to a class and supports removing assignments", () => {
		...
		database.assignStudentsToClass(
			targetClass.id,
		...
		expect(
			database.getAllClassAssignments().students.length,
		...
		expect(
			database
				...
				.teachers.some(
					(assignment) =>
						assignment.class_id === targetClass.id &&
	...
	test("creates, updates and deletes practice records", () => {
		...
		expect(database.getTeacherRecordById(createdRecord.id)?.student_uid).toBe(
			student!.uid,
		);
		expect(
			database.getAllRecords().find((record) => record.id === createdRecord.id)
				?.title,
		).toBe("测试记录");
		expect(
			"content" in
				(database
					.getAllRecords()
					.find((record) => record.id === createdRecord.id) ?? {}),
		).toBe(false);
		expect(
			database
				.getRecordsByStudent(student!.id)
				.some((record) => record.id === createdRecord.id),
		).toBe(true);
		...
		expect(
			database.getStudentStatistics(student!.id).approved_count,
		).toBeGreaterThan(0);
		expect(
			database.getStudentStatistics(student!.id).total_duration,
	...
	test("cleans up replaced and deleted record images", () => {
		...
		const firstImage = createTempUpload(
			`test-record-image-${Date.now()}-1.webp`,
			"first",
		);
		const secondImage = createTempUpload(
			`test-record-image-${Date.now()}-2.webp`,
			"second",
		);
		const thirdImage = createTempUpload(
			`test-record-image-${Date.now()}-3.webp`,
			"third",
		);
		...
		const createdImagePaths = record.image_paths.map((imagePath) =>
			path.join(testUploadDir, path.basename(imagePath)),
		);
		createdImagePaths.forEach((filePath) => {
			cleanupUploadFiles.add(filePath);
		});
		...
		const updatedImagePaths =
			updatedRecord?.image_paths.map((imagePath) =>
				path.join(testUploadDir, path.basename(imagePath)),
			) ?? [];
		updatedImagePaths.forEach((filePath) => {
			cleanupUploadFiles.add(filePath);
		});

		expect(record.image_paths).toHaveLength(2);
		expect(
			record.image_paths.every((imagePath) =>
		...
		expect(database.deleteRecord(record.id, updatedRecord?.image_paths)).toBe(
			true,
		);
	...
	test("keeps reused record images and removes only deleted images", () => {
		...
		const firstImage = createTempUpload(
			`test-record-image-${Date.now()}-keep-1.webp`,
			"first",
		);
		const secondImage = createTempUpload(
			`test-record-image-${Date.now()}-keep-2.webp`,
			"second",
		);
		const thirdImage = createTempUpload(
			`test-record-image-${Date.now()}-keep-3.webp`,
			"third",
		);
		...
		const firstStoredFile = path.join(
			testUploadDir,
			path.basename(record.image_paths[0]!),
		);
		const secondStoredFile = path.join(
			testUploadDir,
			path.basename(record.image_paths[1]!),
		);
		...
		const thirdStoredFile = path.join(
			testUploadDir,
			path.basename(updatedRecord!.image_paths[1]!),
		);
	...
	test("keeps deleted student records visible to the assigned teacher", async () => {
		...
		const teacherRecord = database
			.getAllRecords({}, visibleIds)
	...
	test("tracks unread notifications and aggregate statistics", () => {
		...
		expect(database.getNotificationsByStudent(student!.id)[0]?.id).toBe(
			notification.id,
		);
...
describe("route behavior", () => {
	test("requires users with random initial passwords to set a new password after login", async () => {
		...
		const response = await jsonRequest(
			"/api/auth/login",
			await sealPasswordFields(
				{ uid: String(createdUser.uid), password: createdUser.password },
				["password"],
			),
			{ method: "POST" },
		);
		...
		const changePasswordResponse = await jsonRequest(
			"/api/auth/password",
			await sealPasswordFields(
				{
		...
		expect(decodeJwt(changePasswordPayload.token as string).aud).toBe(
			"student",
		);
		...
		const reloginResponse = await jsonRequest(
			"/api/auth/login",
			await sealPasswordFields(
				{ uid: String(createdUser.uid), password: "new-password-01" },
				["password"],
			),
			{ method: "POST" },
		);
	...
	test("searches login classes and resolves duplicate student name login by selection", async () => {
		...
		await database.updateUserPassword(
			firstStudent!.id,
			await hashPassword("same-pass-01"),
		);
		await database.updateUserPassword(
			secondStudent!.id,
			await hashPassword("same-pass-01"),
		...
		const searchResponse = await apiRequest(
			`/api/auth/classes/search?q=${encodeURIComponent("登录班级")}`,
		);
		const searchPayload = await readJson(searchResponse);
		expect(searchResponse.status).toBe(200);
		expect(
			(searchPayload.classes as Array<{ id: number }>).some(
				(item) => item.id === targetClass.id,
			),
		).toBe(true);

		const loginResponse = await jsonRequest(
			"/api/auth/login/student-name",
			await sealPasswordFields(
				{
		...
		expect(loginPayload.candidates).toMatchObject([
			{
				uid: firstStudent!.uid,
				name: "重名学生",
				english_name: "First Student",
			},
			{
				uid: secondStudent!.uid,
				name: "重名学生",
				english_name: "Second Student",
			},
		]);

		const selectResponse = await jsonRequest(
		...
		expect(selectPayload.user).toMatchObject({
			uid: secondStudent!.uid,
			name: "重名学生",
		});
	});
	...
	test("enforces strong password policy only in production", async () => {
		...
		try {
			...
			const weakResponse = await jsonRequest(
				"/api/auth/password",
				await sealPasswordFields(
					{
						current_password: "teacher-pass-01",
			...
			expect((await readJson(weakResponse)).error).toBe(
				"密码必须包含大写字母、小写字母、数字和特殊符号。",
			);

			appConfig.is_production = false;

			const devResponse = await jsonRequest(
				"/api/auth/password",
				await sealPasswordFields(
					{
						current_password: "teacher-pass-01",
	...
	test("rejects non-image content during upload even if the declared type is allowed", async () => {
		...
		formData.set(
			"image",
			new File(["not really an image"], "fake.png", { type: "image/png" }),
		);
	...
	test("rejects oversized images during upload", async () => {
		...
		formData.set(
			"image",
			new File([oversizedImage], "large.png", { type: "image/png" }),
		);
	...
	test("restricts uploaded images to permitted users", async () => {
		...
		formData.set(
			"image",
			new File([imageBytes], "record.png", { type: "image/png" }),
		);
		...
		cleanupUploadFiles.add(
			path.join(testUploadDir, path.basename(storedImageUrl)),
		);
	...
	test("rejects review comments longer than configured limit", async () => {
		...
		const response = await jsonRequest(
			`/api/teacher/records/${record.id}/review`,
			{
				status: "rejected",
	...
	test("counts only currently active tasks in teacher overview", async () => {
		...
		try {
			...
			const token = await loginAs("1", "admin-pass-01");
			const response = await apiRequest(
				`/api/teacher/overview?class_id=${targetClass.id}`,
				{
	...
	test("returns student dashboard overview with class rank and personal trend", async () => {
		const [
			student,
			higherStudent,
			sameDurationMoreRecordsStudent,
			sameDurationSameRecordsStudent,
		] = await database.createUsers([
		...
		try {
			...
			const mayTrend = (overview.trend as Array<Record<string, unknown>>).find(
				(item) => item.month === "2026-05",
			);
	...
	test("rejects datetime values for practice date filters", async () => {
		...
		const token = await loginAs("1", "admin-pass-01");
		const response = await apiRequest(
			`/api/teacher/records?practice_after=${encodeURIComponent("2026-01-01T00:00:00.000Z")}`,
			{
				headers: { authorization: `Bearer ${token}` },
			},
		);
	...
	test("resubmits rejected record as pending after student edit", async () => {
		...
		const response = await jsonRequest(
			`/api/students/me/records/${record.id}`,
			{
				title: "已修改记录",
				content: "补充后的内容",
				practice_date: "2026-01-13",
	...
	test("keeps existing images when student edit omits image fields", async () => {
		...
		const student = database.findUserByUid(3)!;
		const task = createOpenTaskForStudent(student.id);
		const image = createTempUpload(
			`test-record-image-${Date.now()}-student-keep.webp`,
			"image",
		);
		const record = database.createRecord({
			task_id: task.id,
	...
	test("validates scored task reviews and sorts records by score in the database", async () => {
		...
		const missingScoreResponse = await jsonRequest(
			`/api/teacher/records/${lowRecord.id}/review`,
			{
				status: "approved",
				comment: "",
			},
		...
		const negativeScoreResponse = await jsonRequest(
			`/api/teacher/records/${lowRecord.id}/review`,
			{
				status: "approved",
				score: -1,
		...
		const overflowScoreResponse = await jsonRequest(
			`/api/teacher/records/${lowRecord.id}/review`,
			{
				status: "approved",
				score: 101,
		...
		const decimalScoreResponse = await jsonRequest(
			`/api/teacher/records/${lowRecord.id}/review`,
			{
				status: "approved",
				score: 88.5,
		...
		expect((await readJson(missingScoreResponse)).error).toBe(
			"分数必须是 0 到 100 的整数。",
		);
		...
		const lowReviewResponse = await jsonRequest(
			`/api/teacher/records/${lowRecord.id}/review`,
			{
				status: "approved",
				score: 70,
		...
		const highReviewResponse = await jsonRequest(
			`/api/teacher/records/${highRecord.id}/review`,
			{
				status: "approved",
				score: 95,
			},
		...
		const detailResponse = await apiRequest(
			`/api/teacher/records/${lowRecord.id}`,
			{
				headers: { authorization: `Bearer ${token}` },
			},
		);
		...
		const descResponse = await apiRequest(
			`/api/teacher/records?task_id=${task.id}&sort=score_desc`,
			{
				headers: { authorization: `Bearer ${token}` },
			},
		);
		...
		expect(
			(descPayload.records as Array<{ id: number }>).map((record) => record.id),
		).toEqual([highRecord.id, lowRecord.id, pendingRecord.id]);

		const ascResponse = await apiRequest(
			`/api/teacher/records?task_id=${task.id}&sort=score_asc`,
			{
				headers: { authorization: `Bearer ${token}` },
			},
		);
		...
		expect(
			(ascPayload.records as Array<{ id: number }>).map((record) => record.id),
		).toEqual([lowRecord.id, highRecord.id, pendingRecord.id]);

		const rejectResponse = await jsonRequest(
			`/api/teacher/records/${lowRecord.id}/review`,
			{
				status: "rejected",
				comment: "退回",
		...
		const batchResponse = await jsonRequest(
			"/api/teacher/record-reviews/batch",
			{
		...
		expect((await readJson(batchResponse)).error).toBe(
			"启用打分的任务不能批量通过。",
		);
	});
	...
	test("creates teachers with one selected class from single and batch forms", async () => {
		...
		const singleResponse = await jsonRequest(
			"/api/admin/users",
			{
				name: "单个创建教师",
				role: "teacher",
		...
		expect(
			database.getTeacherClassIds((singlePayload.user as { id: number }).id),
		).toEqual([singleClass.id]);
	...
	test("rejects oversized csv imports in the backend", async () => {
		...
		formData.set(
			"file",
			new File([oversizedCsv], "users.csv", { type: "text/csv" }),
		);
	...
	test("imports csv users with optional student and teacher class name", async () => {
		...
		formData.set(
			...
			new File(
				[
					`CSV 学生,CSV Student,student,${targetClass.name}\nCSV 教师,CSV Teacher,teacher,${targetClass.name}\nCSV 管理员,,admin,\n`,
				],
				"users.csv",
				{ type: "text/csv" },
			),
		...
		expect(response.status).toBe(200);
		expect(payload.users as Array<{ id: number; role: string }>).toHaveLength(
			3,
		);
		expect(payload.credentialsCsv).toContain(
			'"name","english_name","uid","role","password"',
		);
		const student = (payload.users as Array<{ id: number; role: string }>).find(
			(user) => user.role === "student",
		);
		const teacher = (payload.users as Array<{ id: number; role: string }>).find(
			(user) => user.role === "teacher",
		);
		expect(student ? database.getStudentClassId(student.id) : null).toBe(
			targetClass.id,
		);
		expect(teacher ? database.getTeacherClassIds(teacher.id) : []).toContain(
			targetClass.id,
		);
	});
	...
	test("rejects csv class name problems by role", async () => {
		...
		adminClassData.set(
			"file",
			new File(["CSV 管理员,,admin,不存在班级\n"], "users.csv", {
				type: "text/csv",
			}),
		);

		const adminClassResponse = await formRequest(
			"/api/admin/users/import/preview",
			adminClassData,
			{
				method: "POST",
		...
		expect((await readJson(adminClassResponse)).error).toBe(
			"第 1 行错误：管理员不能填写班级。",
		);

		const missingClassData = new FormData();
		missingClassData.set(
			"file",
			new File(["CSV 学生,,student,不存在班级\n"], "users.csv", {
				type: "text/csv",
			}),
		);

		const missingClassResponse = await formRequest(
			"/api/admin/users/import/preview",
			missingClassData,
			{
				method: "POST",
		...
		expect((await readJson(missingClassResponse)).error).toBe(
			"第 1 行错误：班级不存在或错误。",
		);
	});
	...
	test("escapes formula injection in task record export", async () => {
		...
		const response = await jsonRequest(
			`/api/teacher/tasks/${task.id}/export`,
			{ class_ids: [targetClass.id] },
			{
				method: "POST",
				headers: { authorization: `Bearer ${token}` },
	...
	test("locks failed logins per uid and source without blocking other accounts from the same address", async () => {
		...
		for (let index = 0; index < 3; index += 1) {
			const response = await jsonRequest(
				"/api/auth/login",
				await sealPasswordFields({ uid: "3", password: "wrong-password" }, [
					"password",
				]),
				{
					method: "POST",
		...
		const lockedResponse = await jsonRequest(
			"/api/auth/login",
			await sealPasswordFields({ uid: "3", password: "student-pass-01" }, [
				"password",
			]),
			{
				method: "POST",
		...
		const otherUserResponse = await jsonRequest(
			"/api/auth/login",
			await sealPasswordFields({ uid: "4", password: "student-pass-02" }, [
				"password",
			]),
			{
				method: "POST",
...
describe("CSV user import parser", () => {
	test("parses UTF-8 CSV text content", async () => {
		const parsed = await parseUserImportCsvText(
			"张三,,student,\n李老师,,teacher,\n",
			{ columnCount: 4 },
		);
	...
	test("rejects two-column CSV rows", async () => {
		await expect(
			parseUserImportCsvText("张三,student\n", { columnCount: 4 }),
		).rejects.toThrow("第 1 行格式无效，必须包含 4 列。");
	});

	test("parses UTF-16 CSV buffer", async () => {
		const utf16Buffer = new Uint8Array([
			0xff, 0xfe, 0x20, 0x5f, 0x09, 0x4e, 0x2c, 0x00, 0x2c, 0x00, 0x73, 0x00,
			0x74, 0x00, 0x75, 0x00, 0x64, 0x00, 0x65, 0x00, 0x6e, 0x00, 0x74, 0x00,
			0x2c, 0x00, 0x0a, 0x00,
		]);

		const parsed = await parseUserImportCsvBuffer(utf16Buffer, {
			columnCount: 4,
		});
	...
	test("parses GBK CSV buffer", async () => {
		const gbkBuffer = new Uint8Array([
			0xd5, 0xc5, 0xc8, 0xfd, 0x2c, 0x2c, 0x73, 0x74, 0x75, 0x64, 0x65, 0x6e,
			0x74, 0x2c, 0x0a,
		]);

		const parsed = await parseUserImportCsvBuffer(gbkBuffer, {
			columnCount: 4,
		});
	...
	test("rejects unsupported encodings", async () => {
		await expect(
			parseUserImportCsvBuffer(new Uint8Array([0xff, 0xff, 0xff]), {
				columnCount: 3,
			}),
		).rejects.toThrow("无法识别 CSV 文件编码，仅支持 UTF-8、UTF-16 和 GBK。");
	});
});
