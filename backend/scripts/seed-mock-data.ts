...
async function main() {
	...
	for (const teacher of [teacherA, teacherB]) {
		database.updateUserPassword(
			teacher!.id,
			await hashPassword(teacherPassword),
		);
	}
	...
	const recordSeeds = [
		[
			...
			students[0]!,
			"整理社区图书角",
			"今天和同学一起整理社区图书角，重新分类书籍并登记破损图书。服务过程中我学会了和社区工作人员沟通，也发现公共空间需要长期维护。",
			"2026-05-03",
			"社区服务站",
			2,
			"approved",
		...
		[
			...
			students[1]!,
			"协助垃圾分类宣传",
			"我参与了垃圾分类宣传，向居民介绍可回收物和厨余垃圾的区别。过程中有些问题我回答得还不够清楚，后面需要提前准备资料。",
			"2026-05-10",
			"小区广场",
			1.5,
			"pending",
			"2026-05-11T18:10:00+08:00",
		],
		[
			tasks[0]!.id,
			students[2]!,
			"社区活动签到",
			"这次负责社区活动签到和引导，虽然工作简单，但需要耐心核对信息，不能影响活动秩序。",
			"2026-05-18",
			"社区中心",
			1,
			"rejected",
		...
		[
			...
			students[0]!,
			"校园花坛维护",
			"我们清理了花坛杂草，并给植物补水。劳动后我更能理解校园环境维护的不容易。",
			"2026-03-12",
			"教学楼前",
			1.2,
			"approved",
		...
		[
			...
			students[1]!,
			"图书馆书架整理",
			"按照索书号整理书架，发现细致和耐心很重要。",
			"2026-04-02",
			"图书馆",
			1,
			"approved",
			"2026-04-03T17:30:00+08:00",
		],
		[
			tasks[3]!.id,
			students[4]!,
			"青铜器展厅研学",
			"参观青铜器展厅后，我对器物纹样和礼制关系有了更直观的认识。",
			"2026-04-28",
			"市博物馆",
			2.5,
			"approved",
			"2026-04-29T19:00:00+08:00",
		],
		[
			tasks[3]!.id,
			students[5]!,
			"城市记忆展记录",
			"展览用很多老照片呈现城市变化，我记录了几个和生活有关的细节。",
			"2026-05-08",
			"市博物馆",
			2,
			"pending",
			"2026-05-09T20:10:00+08:00",
		],
	] as const;
	...
	for (const seed of recordSeeds) {
		...
		database.updateRecord(record.id, {
			status,
			teacher_comment:
				status === "rejected" ? "内容略短，请补充具体过程。" : null,
